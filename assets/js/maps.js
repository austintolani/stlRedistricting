...
var map4 = L.map('map4').setView([38.633649, -90.243655], 11);
L.esri.basemapLayer("Gray").addTo(map4);

var map5 = L.map('map5').setView([38.633649, -90.243655], 11);
...
L.control.layers(null, overlayMaps).addTo(map3);
L.control.layers(null, overlayMaps).addTo(map4);
L.control.layers(null, overlayMaps).addTo(map5);

wards1.bindPopup(chart, { minWidth: 400 });
wards2.bindPopup(chart, { minWidth: 400 });
wards3.bindPopup(chart, { minWidth: 400 });
...
function chart(d) {
    ...
    var svg = div // select element in the DOM with id 'chart'
        .append('svg') // append an svg element to the element we've selected
        .attr('width', width) // set the width of the svg element we just added
        .attr('height', height) // set the height of the svg element we just added
        .append('g') // append 'g' element to the svg element
        .attr('transform', 'translate(' + (width / 7) + ',' + (height / 2) + ')'); // our reference is now to the 'g' element. centerting the 'g' element to the svg element
    ...
    var votingPopText = div.append('p');
    votingPopText.append('strong').text('Voting Age Population: ');
    votingPopText.append('text').text(`${attributes["votingPop"]}`);

    var compactnessText = div.append('p');
    compactnessText.append('strong').text('Compactness Score (0-1): ');
    compactnessText.append('text').text(`${attributes["ppScore"]}`);
    return div.node();
...
function parkName(d) {
    ...
    return attributes["TEXT_"];
}

function nHoodName(d) {
    var feature = d.feature;
    var attributes = feature.properties;
